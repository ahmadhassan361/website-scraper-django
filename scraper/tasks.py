...
from .scraper_scripts.load_xml_data import load_waterdalecollection_xml_sitemap,load_btshalom_xml_sitemap
import requests
import time
...
def scrape_waterdale_collection(self, session_id, resume_from_index=0):
    ...
    try:
        ...
        # Load product links
        try:
            product_links = load_waterdalecollection_xml_sitemap()
        ...
        # Process products
        for idx, product in enumerate(product_links[start_index:], start=start_index + 1):
            link = product['product_url']
            title_text = product['image_title']
...
# website: btshalom
@shared_task(bind=True, soft_time_limit=7200, time_limit=7260)  # 2 hour soft limit, 2h1m hard limit
def scrape_btshalom(self, session_id, resume_from_index=0):
    """
    Enhanced Celery task to scrape products from  Collection with full logging and state management
    """
    try:
        # Get the scraping session
        session = ScrapingSession.objects.get(id=session_id)
        ...
        # Load product links
        try:
            product_links = load_btshalom_xml_sitemap()
            session.total_products_found = len(product_links)
            session.save()
            log_message(session, 'info', f'Found {len(product_links)} product URLs to scrape')
        except Exception as e:
        ...
        for idx, product in enumerate(product_links[start_index:], start=start_index + 1):
            ...
            try:
                ...
                # Extract product data
                price = soup.select_one('.price-original-wrap .money')
                price_text = price.text.strip() if price else None

                sku = soup.select_one("div.product-sku span[data-product-sku]")
                sku_text = sku.text.strip() if sku else None

                vendor_anchor = soup.select_one("div.product-vendor a")
                category_text = vendor_anchor.text.strip() if vendor_anchor else None

                description_div = soup.select_one('div.product-description.rte')
                description_text = description_div.get_text(separator=' ', strip=True) if description_div else None
...
# website: malchutjudaica
@shared_task(bind=True, soft_time_limit=7200, time_limit=7260)  # 2 hour soft limit, 2h1m hard limit
def scrape_malchutjudaica(self, session_id, resume_from_index=0):
    """
    Enhanced Celery task to scrape products from  Collection with full logging and state management
    """
    try:
            ...
            try:
                ...
                try:
                    ...
                    if created:
                        ...
                        product.save()
                        session.products_updated += 1
                        log_message(session, 'success', f'Updated product: {image_url}', product_url=link, product_sku=sku_text)
